...
execute as @e[type=minecraft:area_effect_cloud,tag=test_room,tag=scan_path] at @s if block ~6 ~ ~12 glass run scoreboard players add @s connection 1
...
execute as @e[type=minecraft:area_effect_cloud,tag=test_room,tag=scan_path] at @s if block ~12 ~ ~6 glass run scoreboard players add @s connection 8

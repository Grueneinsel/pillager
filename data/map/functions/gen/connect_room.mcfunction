...
execute as @e[type=area_effect_cloud,tag=connect_me_next] at @s run scoreboard players operation @s count -= @e[type=area_effect_cloud,tag=entrance,tag=!path_found,tag=!connect_me_next,distance=1..,sort=nearest,limit=1] x
execute as @e[type=area_effect_cloud,tag=connect_me_next] at @s run scoreboard players operation @s count -= @e[type=area_effect_cloud,tag=entrance,tag=!path_found,tag=!connect_me_next,distance=1..,sort=nearest,limit=1] z
execute as @e[type=area_effect_cloud,tag=connect_me_next] if score @s count matches ..-1 run say negative

...
execute if entity @p[tag=debug] if score reset count matches 0..1 if entity @e[type=area_effect_cloud,tag=connect_me_next,tag=!demo] run say at least 1 door left
execute unless entity @p[tag=debug] if score reset count matches 0..1 if entity @e[type=area_effect_cloud,tag=connect_me_next] run function map:gen/connect_door

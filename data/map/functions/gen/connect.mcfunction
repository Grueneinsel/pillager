...
execute as @e[type=area_effect_cloud,tag=entrance,tag=!path_found] at @s rotated as @s positioned ^-6 ^ ^6 if block ~ ~ ~ blue_concrete run tag @s add path_found
execute unless entity @p[tag=debug] if score reset count matches 0..2 if entity @e[type=area_effect_cloud,tag=entrance,tag=!path_found] run function map:gen/connect

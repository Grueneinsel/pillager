...
execute as @e[type=area_effect_cloud,tag=door,tag=path_found,tag=!connected_to_current_room] unless score @s room_id = total room_id if score @s y = current y rotated as @s at @s positioned ^ ^ ^6 run scoreboard players set @e[type=area_effect_cloud,tag=connection,scores={path=3},distance=..7] path 4
...
execute as @e[type=area_effect_cloud,tag=stair_entrance,scores={path=1}] if score @s room_id = total room_id if score @s y = current y at @s rotated as @s positioned ^ ^ ^7 run scoreboard players set @e[type=area_effect_cloud,tag=stair_entrance,distance=..7] path 0
...
execute as @e[type=area_effect_cloud,tag=stair_entrance,tag=connected_to_current_room] rotated as @s positioned ^ ^ ^-6 run scoreboard players set @e[type=area_effect_cloud,tag=stair_entrance,distance=..7] path 0
tag @e[type=area_effect_cloud,tag=stair_entrance,tag=connected_to_current_room] remove door
...
scoreboard players set @s[tag=middle_path] count 1

...
execute if entity @s[tag=4tiles] as @e[type=area_effect_cloud,tag=test_room] at @s if blocks 0 100 0 24 100 24 ~ ~ ~ all run tag @s add pot_room
...
execute if entity @s[tag=4tiles] as @e[type=area_effect_cloud,tag=pot_room,scores={z=0..2}] run tag @s remove pot_room
...
execute at @s[tag=4tiles] run fill ~ ~ ~ ~24 ~ ~24 birch_planks

...
execute if entity @s[tag=stairs] as @e[type=area_effect_cloud,tag=test_room,scores={z=4..}] at @s positioned ~ ~-9 ~ if blocks 0 100 0 10 109 10 ~2 ~ ~2 all unless entity @e[type=area_effect_cloud,tag=entrance,dx=12,dz=12,dy=10] run tag @s add pot_room

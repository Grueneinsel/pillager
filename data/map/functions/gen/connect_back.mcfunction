...
execute as @e[type=area_effect_cloud,tag=pot_connection,distance=..6] run scoreboard players operation total turn < @s turn
execute as @e[type=area_effect_cloud,tag=pot_connection,distance=..6] if score @s turn = total turn run tag @s add reconnect_me

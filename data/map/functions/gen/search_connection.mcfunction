...
##Rotate latest AECs
execute as @e[type=area_effect_cloud,tag=latest_connection] at @s if block ~ ~ ~ blue_concrete run tag @s add blue_block
...
execute unless entity @p[tag=debug] if score reset count matches 0..2 unless entity @s[tag=path_found] if entity @e[type=area_effect_cloud,tag=connect_me] run function map:gen/search_connection

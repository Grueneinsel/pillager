...
execute if score reset count matches 0 unless entity @s[tag=path_found] if entity @e[type=area_effect_cloud,tag=connect_me] run function map:gen/search_connection
execute if score reset count matches 0 unless entity @s[tag=path_found] unless entity @e[type=area_effect_cloud,tag=connect_me] run function map:gen/retry_connection
